...
class Koyomi
{
    ...
    */
    function whatM9inY9($ys9, $m0) {
       ...
       $a = 2 - ($s-1)%3;  //年の一七四→0,九六三→1,八五二→2
       $x = $a*12 + $m +1; //一七四の年の2月=八白 = 9-1 → 1として、+方向にカウント(1,2,...36)
       $x = 9 - ($x % 9);
...
if (!HAVE_ASTRO_SUN):
        ...
        */
        ...
        if ($y < 0) { $y += 1; } //西暦0年は存在しない。計算上は0を含めるようにする
        // $a9=array('二', '一', '九', '八', '七', '六', '五', '四', '三');
        $b9  = array( 2,    1,    9,    8,    7,    6,    5,    4,    3);
        //$a10=array('庚', '辛', '壬', '癸', '甲', '乙', '丙', '丁', '戊', '己');
        $b10 = array( 7,    8,    9,    10,   1,    2,    3,    4,    5,    6);
