...
function doCalc($jisa0, $y0,$m0,$d0, $h0=0, $min0=0, $sec0=0)
{
    ...
    // 指定されたユリウス日の、九星、十干、十二支、旧暦日、六曜得る
    $a = $k->JDto91012($j);
    ...
    $y10 = $a['ynum10'];
    $y12 = $a['ynum12'];
    $ynatt = $k->n1012toNatt($y10,$y12); //納音名を得る
    ...
    $res_str = $res_str . "年: {$s10list[$y10]} {$s12list[$y12]} {$s9list[$y9]} {$ynatt} \n";
    $res_str = $res_str . "月: {$s10list[$m10]} {$s12list[$m12]} {$s9list[$m9]} {$mnatt} \n";
    $res_str = $res_str . "日: {$s10list[$d10]} {$s12list[$d12]} {$s9list[$d9]} {$dnatt} \n";

    // 以下、天文計算ルーチン(moon.php sun.php)が空っぽなので、コメントアウト
    /*
    $qk = $k->JD2Q($j); $dnum6 = $qk['dnum6']; $mnum6 = intval($qk['qm']) % 6 + 1;
    $res_str = $res_str . "旧暦日: {$qk['qy']}-{$qk['qm']}-{$qk['qd']} 月の六曜={$s6list[$mnum6]} 日の六曜={$s6list[$dnum6]} \n";
